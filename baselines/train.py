...
Usage:
    python train.py
    python train.py --epochs 100 --lr 5e-4 --batch 32
"""
...
def train_one_epoch(head, loader, optimizer, device):
    head.train()
    loader.dataset.reset_progress()
    total_loss, total_nll, total_crps, n_batches = 0.0, 0.0, 0.0, 0

    for batch in loader:
        beetle = batch["beetle_emb"].to(device)
        color = batch["colorpicker_emb"].to(device)
        scale = batch["scalebar_emb"].to(device)
        species = batch["species_idx"].to(device)
...
@torch.no_grad()
def validate(head, loader, device, target_stds):
    """Compute loss + per-target CRPS in original scale."""
    head.eval()
    loader.dataset.reset_progress()
    total_loss, total_nll, total_crps, n_batches = 0.0, 0.0, 0.0, 0

    # Accumulate per-target CRPS in original scale for the RMS metric
    crps_sums = torch.zeros(3)
    n_events = 0
    ...
    for batch in loader:
        ...
        # CRPS in original scale (Section 10)
        mu_real = mu * ts
        sigma_real = sigma * ts
        y_real = targets * ts
        crps_orig = gaussian_crps(mu_real, sigma_real, y_real)  # [B, 3]
        crps_sums += crps_orig.sum(dim=0).cpu()
        n_events += targets.shape[0]

    per_target_crps = crps_sums / n_events  # [3]
...
@torch.no_grad()
def calibrate_sigma(head, loader, device, target_stds):
    """Grid-search α ∈ [0.7, 1.5] per target to minimize CRPS."""
    head.eval()
    loader.dataset.reset_progress()
    ...
    for batch in loader:
        ...
        mu, sigma = head(beetle, color, scale, species, mask)
        all_mu.append(mu * ts)
        all_sigma.append(sigma * ts)
        all_y.append(targets * ts)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            mu_real = mu * ts
            sigma_real = sigma * ts * sc
            y_real = targets * ts
            crps_orig = gaussian_crps(mu_real, sigma_real, y_real)
            crps_sums += crps_orig.sum(dim=0).cpu()
            n_events += targets.shape[0]

    per_target = crps_sums / n_events
